...
int main(int argc, char *argv[]) {
    ...
    uint64_t t_phys_addrs[num_accesses];
    uintptr_t t_virt_addrs[num_accesses];
    ...
    if (cas_fd >= 0 && read(cas_fd, &cas_before, sizeof(uint64_t)) != sizeof(uint64_t)) {
        fprintf(stderr, "Failed to read CAS counter (before)\n");
    }
    ...
    if (cas_fd >= 0 && read(cas_fd, &cas_after, sizeof(uint64_t)) == sizeof(uint64_t)) {
        close(cas_fd);
        printf("\n[+] Uncore CAS Reads (IMC0): %lu → %lu (Δ = %lu)\n", cas_before, cas_after, cas_after - cas_before);
    }
